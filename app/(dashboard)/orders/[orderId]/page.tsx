...
const OrderDetails = async ({ params }: { params: { orderId: string } }) => {
  ...
  const { orderDetails, products, customer } = await res.json();
  ...
  return (
    <div className="flex flex-col p-10 gap-5">
      <p className="text-base-bold">
        Order ID: <span className="text-base-medium">{orderDetails.id}</span>
      </p>
      <p className="text-base-bold">
      ...
      <DataTable columns={columns} data={products} searchKey="productTitle" />
    </div>
  );
};
